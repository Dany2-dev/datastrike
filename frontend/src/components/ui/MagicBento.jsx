...
const cardData = [
  ...
  },
  {
    ...
    label: 'Efficiency',
    details: 'Listado completo de rendimiento individual procesado en tiempo real.'
  },
  {
    color: '#060010',
    title: 'xG',
  ...
  },
  {
    ...
    label: 'Protection',
    details: 'Encriptación de grado militar para datos sensibles de contratos y scouting. Cumple con los estándares GDPR.'
  }
];
...
const createParticleElement = (x, y, color = DEFAULT_GLOW_COLOR) => {
  ...
  el.style.cssText = `
    ...
    background: rgba(${color}, 1); box-shadow: 0 0 8px rgba(${color}, 0.8);
    pointer-events: none; z-index: 100; left: ${x}px; top: ${y}px; will-change: transform, opacity;
  `;
  return el;
};
...
const ParticleCard = ({
  ...
  const clearAllParticles = useCallback(() => {
    ...
    particlesRef.current.forEach(p => {
      gsap.to(p, { scale: 0, opacity: 0, duration: 0.4, onComplete: () => p.parentNode?.removeChild(p) });
    });
    ...
    memoizedParticles.current.forEach((particle, index) => {
      const timeoutId = setTimeout(() => {
        ...
        gsap.to(clone, {
          ...
          rotation: Math.random() * 720, duration: 3 + Math.random() * 3,
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => { if (!isExpanded) { isHoveredRef.current = true; animateParticles(); } };
    const handleMouseLeave = () => {
      isHoveredRef.current = false; clearAllParticles();
      gsap.to(element, { rotateX: 0, rotateY: 0, x: 0, y: 0, duration: 0.6 });
    };
    const handleMouseMove = e => {
      ...
      if (enableTilt) {
        ...
        gsap.to(element, { rotateX, rotateY, duration: 0.2, transformPerspective: 1200 });
      }
      if (enableMagnetism) {
        gsap.to(element, { x: (x - centerX) * 0.04, y: (y - centerY) * 0.04, duration: 0.4 });
      }
    };
  ...
  return (
    <div ref={cardRef} onClick={onClick} style={style}
...
const GlobalSpotlight = ({ gridRef, isAnyCardExpanded, glowColor, spotlightRadius }) => {
  ...
  useEffect(() => {
    if (isAnyCardExpanded) { if (spotlightRef.current) spotlightRef.current.style.opacity = '0'; return; }
    ...
    const handleMouseMove = e => {
      ...
      cards.forEach(card => {
        ...
        const effectiveDist = Math.max(0, dist); minDistance = Math.min(minDistance, effectiveDist);
      ...
      gsap.to(spotlight, { left: e.clientX, top: e.clientY, opacity: minDistance < 100 ? 1 : 0, duration: 0.3 });
    };
...
const MagicBento = ({ equipoId, initialData, ...props }) => {
  ...
  return (
    <div className="bento-wrapper">
      <GlobalSpotlight 
        gridRef={gridRef} isAnyCardExpanded={activeCardIndex !== null}
      ...
      <div className="card-grid bento-section" ref={gridRef}>
        {cardData.map((card, index) => {
          ...
          return (
            <ParticleCard
              key={index} isExpanded={isExpanded}
              ...
              <div className="magic-bento-card__header">
                ...
                {isExpanded && (
                  <button className="magic-bento-card__close" onClick={(e) => {
                    e.stopPropagation(); setActiveCardIndex(null);
                  }}>✕</button>
                )}
              </div>

              <div className="magic-bento-card__content" onClick={(e) => isExpanded && e.stopPropagation()}>
                ...
                {isExpanded && (
                  <div className="magic-bento-card__extended-info">
                    ...
                    {card.title === "PLAYER STATS" && (
                      <div className="player-table-wrapper" style={{ marginTop: '20px', width: '100%' }}>
                        {equipoId ? (
                          <PlayerStatsTable 
                            equipoId={equipoId} 
                          ...
                          <p style={{color: '#888', textAlign: 'center'}}>No se ha detectado un ID de equipo válido.</p>
                        )}
                      </div>
                    )}
                  </div>
